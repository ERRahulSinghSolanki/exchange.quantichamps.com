...
function ActionCell({ id, quest_code }) {
  ...
  const [lastObjectCount, setLastObjectCount] = useState(0);
  ...
  const handleOpen = async () => {
    ...
    try {
      ...
      if (Array.isArray(apiResponseQuotas)) {
        apiResponseQuotas.forEach((group) => {
          if (Array.isArray(group)) {
            group.forEach((item) => {
              totalCount += item.count; // Safely add the count value
            });
          }
        });
      }
  ...
  return (
    <SoftBox display="flex" alignItems="center">
      ...
      <SoftTypography variant="body1" mr={1} color="primary" sx={{ cursor: "pointer", lineHeight: 0 }} onClick={handleStudies}>
        <Tooltip title="Studies" placement="top">
          <Icon>visibility</Icon>
        </Tooltip>
      </SoftTypography>
      ...
      <Dialog open={isModalOpen} onClose={handleCloseModal}  PaperProps={{ sx: {width: '70vw',height: '85vh',maxWidth: 'none',padding: '0px 10px'} }}>
        ...
        <DialogTitle sx={{ textAlign: "center", fontSize: "22px" }}>
          ...
          <SoftBox lineHeight={1} sx={{ textAlign: "left", fontSize: "18px", marginTop: "10px" }} color="primary" fontWeight="medium">
            ...
            <span style={{ color: "#344767" }}>Quotas Count :</span> {lastObjectCount}
          </SoftBox>
        </DialogTitle>
