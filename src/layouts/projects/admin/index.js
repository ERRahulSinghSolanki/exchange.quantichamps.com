...
const LoginApprovalNotifications = () => {
  const [notifications, setNotifications] = useState([]);
  const [snackbar, setSnackbar] = useState({ open: false, message: "", color: "success" });
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [comments, setComments] = useState({});
  const [processing, setProcessing] = useState({}); // Track processing state for each notification

  const fetchNotifications = async () => {
    try {
      const token = localStorage.getItem("authToken");

      const earlyRes = await axios.get(`${API_URL}/admin/early-login-notifications`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      const lateRes = await axios.get(`${API_URL}/admin/late-login-notifications`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleApproval = async (attendanceId, notificationId, isApproved, type) => {
    try {
      setProcessing(prev => ({ ...prev, [notificationId]: true })); // Set processing state for this notification
      const token = localStorage.getItem("authToken");

      const url =
        type === "early"
          ? `${API_URL}/admin/early-login-approve/${attendanceId}`
          : `${API_URL}/admin/late-login-approve/${attendanceId}`;
      ...
      setProcessing(prev => ({ ...prev, [notificationId]: false })); // Reset processing state
    }
  };
            ...
            {paginatedData.map((n) => (
              ...
              >
                ...
                <TextField
                  ...
                  sx={{ mt: 2 }}
                  disabled={processing[n.id]} // Disable when processing
                />
                ...
                <SoftBox mt={2}>
                  <SoftButton
                    ...
                    disabled={processing[n.id]} // Disable when processing
                  >
                    {processing[n.id] ? (n.status === "pending_early_approval" ? "Approving Early..." : "Approving Late...") : "Approve"}
                  </SoftButton>
                  <SoftButton
                    color="error"
                    size="small"
                    ...
                    disabled={processing[n.id]} // Disable when processing
                  >
                    {processing[n.id] ? "Rejecting..." : "Reject"}
                  </SoftButton>
                </SoftBox>
              </SoftBox>
            ))}
...
export default LoginApprovalNotifications;