...
const UsersList = () => {
  ...
  const handleDeleteClick = (id) => {
    setDeleteDialog({ open: true, userId: id, loading: false });
  };
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/users/${deleteDialog.userId}`, {
        method: "DELETE",
  ...
  const allRows = useMemo(() => users.map((user) => {
    ...
    return {
      ...
      shift_time: <SoftTypography variant="button" fontWeight="medium">
      ...
      status: <SoftTypography variant="button" fontWeight="medium" color={user.deleted_at ? "error" : "success"}>
        {user.deleted_at ? "Inactive" : "Active"}
      </SoftTypography>,
      action: (
        <SoftBox display="flex" justifyContent="center">
          <SoftButton size="small" color="info" sx={{ mx: 0.5 }} onClick={() => navigate(`/users/edit/${user.id}`)}>
            Edit
          </SoftButton>
          {user.deleted_at ? (
            <SoftButton size="small" color="success" sx={{ mx: 0.5 }} onClick={() => handleRestore(user.id)}>
              Restore
            </SoftButton>
          ) : (
            <SoftButton size="small" color="error" sx={{ mx: 0.5 }} onClick={() => handleDeleteClick(user.id)}>
              Delete
            </SoftButton>
          )}
        </SoftBox>
  ...
  return (
    <DashboardLayout>
      ...
      <Dialog
        ...
        onClose={!deleteDialog.loading ? handleDeleteCancel : null}
      ...
      >
        ...
        <DialogActions>
          <SoftButton onClick={handleDeleteCancel} disabled={deleteDialog.loading}>
            Cancel
          </SoftButton>
          <SoftButton onClick={handleDeleteConfirm} color="error" disabled={deleteDialog.loading} autoFocus>
            {deleteDialog.loading ? <CircularProgress size={24} color="inherit" /> : "Delete"}
          </SoftButton>
        </DialogActions>
      </Dialog>
      ...
      <SoftBox p={3}>
        <SoftBox mb={3}>
          <SoftBox display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            ...
            <SoftButton variant="gradient" color="dark" onClick={() => navigate("/users/add")}>
              Add New User
            </SoftButton>
          </SoftBox>

          <SoftBox display="flex" justifyContent="center" alignItems="center" my={2} gap={1}>
            <SoftButton size="small" variant="outlined" onClick={() => handlePageChange(1)} disabled={pagination.page === 1}>
              {"<<"}
            </SoftButton>
            <SoftButton size="small" variant="outlined" onClick={() => handlePageChange(pagination.page - 1)} disabled={pagination.page === 1}>
              {"<"}
            </SoftButton>
            <input 
              type="number" 
