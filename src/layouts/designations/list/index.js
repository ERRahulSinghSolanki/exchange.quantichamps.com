...
const PERMISSION_PREFIX = "viewDesignation.";  // Changed to designation permissions
...
const DesignationList = () => {
  ...
  const fetchDesignations = async () => {
    try {
      ...
      if (!res.ok) { 
        ...
        throw new Error(err.message || "Failed to fetch designations"); 
      }
      ...
      setSnackbar({ 
        open: true, 
        message: err.message || "Error loading designations", 
        color: "error" 
      });
    } finally { 
      setLoading(false); 
  ...
  const handleEditClick = (designation) => navigate(`/designations/edit/${designation.id}`);
  const handleDeleteConfirm = async () => {
    try {
      setDeletingId(selectedDesignation.id);
      const token = localStorage.getItem("authToken");
      const res = await fetch(`${API_URL}/designations/${selectedDesignation.id}`, {
        method: "DELETE", 
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) { 
    ...
    <DashboardLayout>
      ...
      <SoftBox p={3}>
        <SoftBox display="flex" justifyContent="space-between" alignItems="center" mb={3}>
          <SoftTypography variant="h4" fontWeight="bold">Designation List</SoftTypography>
          <SoftButton
            variant="gradient" 
            color="info" 
                            ...
                            switch (col.key) {
                              ...
                              case "created_at":
                                return (
                                  ...
                                  >
                                    {new Date(desg.created_at).toLocaleString()}
                                  </TableCell>
                                );
                              case "status":
                                return (
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        ...
        <DialogContent>
          <SoftTypography variant="body2">
            Are you sure you want to delete the designation <strong>{selectedDesignation?.name}</strong>?
          </SoftTypography>
        </DialogContent>
        <DialogActions>
          <SoftButton
            ...
            disabled={deletingId === selectedDesignation?.id}
            sx={{ 
          ...
          >
            {deletingId === selectedDesignation?.id ? 
