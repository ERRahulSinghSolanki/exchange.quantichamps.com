...
const ManagerLoginApprovalNotifications = () => {
  const [notifications, setNotifications] = useState([]);
  const [snackbar, setSnackbar] = useState({ open: false, message: "", color: "success" });
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [comments, setComments] = useState({});
  const [processingStates, setProcessingStates] = useState({}); // Track processing state for each notification

  const fetchNotifications = async () => {
    try {
      const token = localStorage.getItem("authToken");

      const earlyRes = await axios.get(`${API_URL}/manager/early-login-notifications`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      const lateRes = await axios.get(`${API_URL}/manager/late-login-notifications`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleApproval = async (attendanceId, notificationId, isApproved, type) => {
    try {
      ...
      setProcessingStates(prev => ({ 
        ...prev, 
        [notificationId]: isApproved ? "approving" : "rejecting" 
      }));
      ...
      let url = "";
      if (type === "early") {
        url = `${API_URL}/manager/early-login-approve/${attendanceId}`;
      } else if (type === "late") {
        url = `${API_URL}/manager/late-login-approve/${attendanceId}`;
      }
  ...
  return (
    <DashboardLayout>
      ...
      <SoftSnackbar
        ...
        icon="notifications"
        title="Manager Login Approval Requests"
      ...
      <SoftBox p={3}>
        ...
        {loading ? (
          ...
          <>
            {paginatedData.map((n) => {
              const isProcessing = Boolean(processingStates[n.id]);
              const isApproving = processingStates[n.id] === "approving";
              const isRejecting = processingStates[n.id] === "rejecting";
...
export default ManagerLoginApprovalNotifications;